  ...
  return (
    <Box sx={{ maxWidth: 680 }}>
          ...
          <DetailRow
            ...
            value={
              city.is_popular != null ? (
                <Chip
                  label={city.is_popular ? "Da" : "Ne"}
                  size="small"
                  color={city.is_popular ? "success" : "default"}
                />
              ) : null
            }
          />
            ...
            >
              {categories.map((cat) => (
                <Box
                  key={cat.id}
                  onClick={() =>
                    navigate(`/gradovi/${id}/kategorije/${cat.id}`)
                  }
                  sx={{
                    p: 2,
                    borderRadius: 2,
      ...
      <Dialog open={confirmOpen} onClose={() => setConfirmOpen(false)}>
        ...
        <DialogContent>
          <DialogContentText>
            Da li si siguran da želiš da obrišeš grad{" "}
            <strong>{city?.name}</strong>? Ova akcija se ne može poništiti.
          </DialogContentText>
        </DialogContent>
